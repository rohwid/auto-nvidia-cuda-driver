...
echo " + Latest CUDA installers [ex: cuda_9.0.176_384.81_linux.run]."
...
if [[ -L /usr/local/cuda ]]; then
  ...
  if [[ -f /usr/local/cuda/include/cudnn.h ]]; then
    echo "[AUTO-CUDA] CUDNN has installed in your system."
  fi
...
read -p "Please specify CUDA installers file. [Defaut: cuda_9.0.176_384.81_linux.run]: " CUDA
CUDA="${CUDA:=cuda_9.0.176_384.81_linux.run}"
read -p "Please specify CDNN installers file. [Defaut: cudnn-9.0-linux-x64-v7.4.2.24.tgz]: " CUDNN
CUDNN="${CUDNN:=cudnn-9.0-linux-x64-v7.4.2.24.tgz}"
read -p "Please specify NCCL installers file. [Defaut: nccl_2.3.7-1+cuda9.0_x86_64.txz]: " NCCL
