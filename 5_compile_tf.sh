...
if [ ! -d tensorflow_pkg ]; then
  ...
  if [ $COMPL = "N" ] || [ $COMPL = "n" ]; then
    ...
    echo "[CUDA-TSFLOW] Deleting previous compilation."
    rm -r tensorflow_pkg
    echo "[CUDA-TSFLOW] Previous compilation successfully deleted."
    echo "[CUDA-TSFLOW] Please re-execute this script!"
    exit
  fi
fi
